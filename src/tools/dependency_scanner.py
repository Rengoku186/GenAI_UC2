...
def parse_file_chunks(file_path: str) -> List[Tuple[str, str, str, str, int, int]]:
    ...
    if lang == 'cobol':
        ...
        # Find paragraphs in PROCEDURE DIVISION
        proc_div_idx = 0
        ...
        # Find VB Attribute VB_Name or file basename
...
def scan_project(file_paths: List[str]) -> nx.DiGraph:
    graph = nx.DiGraph()
    chunk_registry: Dict[str, str] = {} # name -> qualified_id
    all_chunks_info = []
    ...
    for path in file_paths:
        ...
        for qid, scope, name, code, s_line, e_line in parsed_chunks:
            ...
            all_chunks_info.append((qid, path, detect_language(path), code, name))
    ...
    for qid, path, lang, code, current_name in all_chunks_info:
        ...
        for m in matches:
            ...
            if called_upper in chunk_registry:
                ...
                # Try prefixing with current file path
