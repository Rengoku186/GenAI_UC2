...
public class DemoApplication {
    ...
    public static void main(String[] args) {
        try {
            ...
            ChequeProcessor chequeProcessor = new ChequeProcessor(currencyExchangeService, signatureVerificationService,
                    coreBankingSystemUpdater, chequeHistoryManager, fraudDetectionService, exceptionReportManager,
                    chequeStatusManager, emailNotificationService);
            ...
            while (authenticatedUser != null) { // Continue loop only if authenticated
                ...
                switch (choice) {
                    ...
                    case 6:
                        handleChequeImageSubmission(scanner, imageHandler, cryptoService, clearinghouseService,
                                authenticatedUser);
                        break;
                    ...
                    case 12:
                        ...
                        exceptionReportManager.recordFIRDetails(firAccount, firCheque, firNumber, policeStation,
                                firDate, firRemarks);
                        break;
    ...
    private static void processChequeBatch(Scanner scanner, ChequeProcessor chequeProcessor) {
        try {
            ...
            chequesToProcess.forEach(cheque -> {
                try {
                    chequeProcessor.processCheque(cheque.accountNumber, cheque.chequeNumber, cheque.currency,
                            cheque.amount, cheque.signature);
                } catch (Exception ex) {
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 3:
                    ...
                    Map<String, Double> detailedRates = currencyExchangeService
                            .getDetailedExchangeRates(detailCurrencyCode);
    ...
    private static void handleReportGeneration(Scanner scanner, ChequeHistoryManager chequeHistoryManager) {
        ...
        List<ChequeHistoryManager.ChequeRecord> records = chequeHistoryManager.getAllChequeRecordsInPeriod(startDate,
                endDate);
    ...
    /**
     * Handles the process of "scanning", encrypting, signing, and "sending" a
     * cheque image.
     * 
    ...
    private static void handleChequeImageSubmission(Scanner scanner,
        ...
        System.out.println("Encrypted image data signed. Signature: " + digitalSignature.substring(0, 10) + "..."); // Show
                                                                                                                    // partial
                                                                                                                    // signature
    ...
    static class ChequePrintingService {
        public void printCheque(String payeeName, double amount, Date date, String accountNumber, String chequeNumber,
                String bankName) {
    ...
    class BatchCheque {
        ...
        public BatchCheque(String accountNumber, String chequeNumber, String currency, double amount,
                String signature) {
    ...
    static class ExceptionReportManager {
        ...
        public boolean recordFIRDetails(String accountNumber, String chequeNumber, String firNumber,
                String policeStation, Date firDate, String remarks) {
    ...
    static class ChequeStatusManager {
        ...
        public void setStatus(String accountNumber, String chequeNumber, ChequeStatus status) {
            ...
            System.out.println(
                    "Status of cheque " + chequeNumber + " for account " + accountNumber + " set to " + status);
        }
    ...
    static class ChequeImageHandler {
         ...
         * In a real application, this would involve actual file I/O and image
         * processing.
         * 
        ...
        public byte[] loadImageData(String filePath) {
            ...
            // Simulate reading file content. For this demo, we'll just use the path string
            // as bytes.
    ...
    static class CryptographyService {
        ...
        public byte[] encryptData(byte[] data, String key) {
            // This is a placeholder. Real encryption would use JCA (Java Cryptography
            // Architecture).
        ...
        public String signData(byte[] data, String privateKey) {
            // This is a placeholder. Real digital signatures use algorithms like RSA,
            // ECDSA.
            // For simulation, create a simple hash-like string.
            int hashCode = Arrays.hashCode(data);
            return "signature(" + privateKey + "):" + Integer.toHexString(hashCode)
    ...
    static class ClearinghouseService {
         ...
         */
        public void submitToClearinghouse(String accountNumber, String chequeNumber, byte[] encryptedImageData,
            ...
            System.out.println("Digital Signature: "
                    + digitalSignature.substring(0, Math.min(digitalSignature.length(), 20)) + "..."); // Show partial
                                                                                                       // signature
    ...
    static class SignatureVerificationService {
        ...
        public boolean verifySignature(String accountNumber, String signature) {
            ...
            // If we don't have a signature on file, accept any signature (for demo
            // purposes)
    ...
    static class ChequeProcessor {
        ...
        public void processCheque(String accountNumber, String chequeNumber, String currency, double amount,
                String signature) {
            try {
                ...
                if (!signatureVerificationService.verifySignature(accountNumber, signature)) {
                    exceptionReportManager.reportException(accountNumber, chequeNumber, "Altered",
                            "Signature mismatch");
                    ...
                    emailNotificationService.sendEmail(
                            ...
                            "Cheque " + chequeNumber + " for account " + accountNumber
                ...
                if (fraudDetectionService.isFraudulentCheque(accountNumber, chequeNumber, amount)) {
                    exceptionReportManager.reportException(accountNumber, chequeNumber, "Duplicate",
                            "Fraudulent or duplicate cheque detected");
                ...
                if (amount > 50000) {
                    exceptionReportManager.reportException(accountNumber, chequeNumber, "Bounced",
                    ...
                    emailNotificationService.sendEmail(
                            ...
                            "Cheque " + chequeNumber + " for account " + accountNumber
                                    + " has bounced due to insufficient funds.");
                    return;
                }

                // Simulate delayed cheque (for demo, if cheque number ends with '9')
                if (chequeNumber.endsWith("9")) {
                    exceptionReportManager.reportException(accountNumber, chequeNumber, "Delayed",
                            "Cheque processing delayed (simulated)");
                ...
                emailNotificationService.sendEmail(
                        ...
                        "An error occurred while processing cheque " + chequeNumber + " for account " + accountNumber
                                + ": " + ex.getMessage());
            }
        }
    ...
    static class CurrencyExchangeServiceV2 {
        ...
        private double fetchRateFromAPIV2(String currency) throws Exception {
            ...
            int end = start;
            while (end < all.length() && (Character.isDigit(all.charAt(end)) || all.charAt(end) == '.'))
                end++;
    ...
    static class FraudDetectionService {
        ...
        private AlertLevel determineAlertLevel(boolean isDuplicate, boolean isAbnormal,
            ...
            int fraudCount = 0;
            if (isDuplicate || isHistoricalDuplicate)
                fraudCount += 3;
            if (isAbnormal)
                fraudCount += 2;
            if (isSuspicious)
    ...
    /**
     * Core fraud detection class that implements fundamental fraud detection
     * mechanisms.
     */
    static class FraudDetection {
        ...
        public boolean isSuspiciousActivity(String accountId, double amount) {
            double totalActivity = accountActivity.containsKey(accountId) ? accountActivity.get(accountId) : 0.0;
...
public class ChequeApplication {
    ...
    static class ChequeHistoryManager {
        ...
        public void displayChequeHistory(String acc) {
            System.out.println("History for " + acc + ": " + history.getOrDefault(acc, Collections.emptyList()).size()
                    + " records.");
        }

        public List<String> getChequeNumbers(String acc) {
            return new ArrayList<>();
        }

        public int getTotalChequeCount(String acc) {
            return 0;
        }

        public int getRecentChequeCount(String acc) {
            return 0;
        }

        public boolean hasSimilarRecentCheque(String acc, double amt, double threshold) {
            return false;
        }

        public List<ChequeRecord> getAllChequeRecordsInPeriod(LocalDate start, LocalDate end) {
            return new ArrayList<>();
        }
...
public class CreateApplication {
    ...
    private static void processChequeBatch(Scanner scanner, ChequeProcessor chequeProcessor) {
        ...
        System.out.println("\nProcessing batch...");
        chequesToProcess.forEach(cheque -> chequeProcessor.processCheque(cheque.accountNumber, cheque.chequeNumber,
                cheque.currency, cheque.amount, cheque.signature));
    }
